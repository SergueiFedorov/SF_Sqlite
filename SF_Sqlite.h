...
public:
	...
	const SF_CODES::ERRORS execute(const std::string& query, const std::vector<SF_Sqlite_Parameter>& params, std::list<SF_Sqlite_Row>& result) const;
	
	//DOES: Executes a scalar query which returns a single int result
	//ERRORS: NOT_CONNECTED, QUERY_FAIL
	//INPUT QUERY: SELECT count(*) FROM Example
	const SF_CODES::ERRORS executeScalar(const std::string& query, int& result) const;

	//DOES: Executes a scalar query which returns a single int result
	//ERRORS: NOT_CONNECTED, QUERY_FAIL
	//INPUT QUERY: SELECT count(*) FROM Example
	const SF_CODES::ERRORS executeScalar(const std::string& query, char& result) const;
	...
	const SF_CODES::ERRORS executeCount(const std::string& query, int& result);

	//DOES: Inserts values into table
	//ERRORS: NOT_CONNECTED, QUERY_FAIL
	//OUTPUT QUERY: INSERT INTO Example VALUES ( 0, 'Serguei Fedorov')
	const SF_CODES::ERRORS insertRecord(const std::string& table, const std::vector<std::string>& values) const;
	...
	const SF_CODES::ERRORS insertRecord(const std::string& table, const std::vector<SF_Sqlite_Column_Data_Pair>& values) const;
